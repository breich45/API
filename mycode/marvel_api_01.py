...
def main():
    ...
    result = marvelcharcall(rand, keyhash, pubkey, char)  # search for Wolverine

    ## display results
    #pprint(result)
    print(f"Name: {result['data']['results'][0]['name']}")
    print(f"Description: {result['data']['results'][0]['description']}")
    comics = []
    for i in result['data']['results'][0]['comics']['items']:
